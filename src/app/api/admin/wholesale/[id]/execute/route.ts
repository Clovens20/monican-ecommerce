...
/**
 * POST /api/admin/wholesale/[id]/execute
 * Exécute une commande wholesale : vérifie le stock, réduit le stock, marque comme complétée
 */
export async function POST(
    ...
    try {
        ...
        // Réduire le stock pour chaque article
        for (const item of items) {
            ...
            if (!reduced) {
                // Rollback partiel non implémenté - en cas d'échec, on retourne une erreur
                return NextResponse.json({
                    success: false,
        ...
        console.error('Error in wholesale execute:', error);
        return NextResponse.json(
